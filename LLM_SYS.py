...
class BioBrainAgent:
    ...
    def extract_entities_with_reflection(self, text,use_reflection):
        """智能体工作流：提取包含标准名和别名的实体字典"""
        prompt_s1 = """你是一个生物学实体识别专家。请从给定的文献中提取出所有的核心生物实体（如基因、蛋白质、细胞过程等）。
        特别注意：不同作者可能使用不同的缩写、全称或别名。你需要为每个实体确定一个【标准名称 (standard_name)】，并把文中出现的其他表达作为【别名 (aliases)】
        标准名称要求简单便于显示不宜太长正常为4个单词以内，有特殊需求时除外，必要时可以重新组织语言，并将原来的标准名称加到别名中。
        
        请严格只输出一个合法的 JSON 数组，绝对不要有其他废话。格式必须如下：（仅作格式参考）
        [
          {
            "standard_name": "p53",
            "aliases": ["tumor suppressor protein p53", "TP53"]
          },
          {
            "standard_name": "apoptosis",
            "aliases": ["cellular apoptosis", "programmed cell death"]
          }
        ]"""
        ...
        prompt_s1_5 = """你是一个严谨的知识图谱数据清洗专家。
        用户将提供一段【原始文献】和一份【初次提取的实体字典】。
        你的任务是：
        1. 查漏补缺：是否有关键的生物学实体被完全遗漏了？
        2. 实体去重合并（最重要）：初次提取的字典中，是否把同一个事物的不同叫法当成了两个不同的实体？如果是，请把它们合并成一个，把多余的名称放入 aliases 中。
        3. 如果标准名称过长，可以适当缩短概括，并将原来的标准名称放入别名中
        
        请直接输出清洗、合并、补全后的最终 JSON 数组，保持与初次提取相同的格式。绝对不要有其他废话。"""
